...
public class WorkOrderTimeEntryService {
    ...
    public List<WorkOrderTimeEntryDTO> findAll() {
        final List<WorkOrderTimeEntry> workOrderTimeEntries = workOrderTimeEntryRepository.findAll(Sort.by("id"));
    ...
    public WorkOrderTimeEntryDTO get(final UUID id) {
        return workOrderTimeEntryRepository.findById(id)
                .map(workOrderTimeEntry -> mapToDTO(workOrderTimeEntry, new WorkOrderTimeEntryDTO()))
    ...
    public void update(final UUID id, final WorkOrderTimeEntryDTO workOrderTimeEntryDTO) {
        final WorkOrderTimeEntry workOrderTimeEntry = workOrderTimeEntryRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        mapToEntity(workOrderTimeEntryDTO, workOrderTimeEntry);
    ...
    private WorkOrderTimeEntryDTO mapToDTO(final WorkOrderTimeEntry workOrderTimeEntry,
        ...
        workOrderTimeEntryDTO.setWorkOrder(workOrderTimeEntry.getWorkOrder() == null ? null : workOrderTimeEntry.getWorkOrder().getId());
        workOrderTimeEntryDTO.setEngineerUser(workOrderTimeEntry.getEngineerUser() == null ? null : workOrderTimeEntry.getEngineerUser().getId());
        return workOrderTimeEntryDTO;
    }

    private WorkOrderTimeEntry mapToEntity(final WorkOrderTimeEntryDTO workOrderTimeEntryDTO,
        ...
        final WorkOrder workOrder = workOrderTimeEntryDTO.getWorkOrder() == null ? null : workOrderRepository.findById(workOrderTimeEntryDTO.getWorkOrder())
                .orElseThrow(() -> new NotFoundException("workOrder not found"));
        workOrderTimeEntry.setWorkOrder(workOrder);
        final User engineerUser = workOrderTimeEntryDTO.getEngineerUser() == null ? null : userRepository.findById(workOrderTimeEntryDTO.getEngineerUser())
    ...
    public void on(final BeforeDeleteWorkOrder event) {
        ...
        final WorkOrderTimeEntry workOrderWorkOrderTimeEntry = workOrderTimeEntryRepository.findFirstByWorkOrderId(event.getId());
    ...
    public void on(final BeforeDeleteUser event) {
        final ReferencedException referencedException = new ReferencedException();
        final WorkOrderTimeEntry engineerUserWorkOrderTimeEntry = workOrderTimeEntryRepository.findFirstByEngineerUserId(event.getId());
