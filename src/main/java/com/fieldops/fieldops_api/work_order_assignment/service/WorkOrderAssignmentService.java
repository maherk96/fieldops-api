...
public class WorkOrderAssignmentService {
  ...
  private WorkOrderAssignmentDTO mapToDTO(
    ...
    workOrderAssignmentDTO.setWorkOrder(
        workOrderAssignment.getWorkOrder() == null
            ? null
            : workOrderAssignment.getWorkOrder().getId());
    workOrderAssignmentDTO.setEngineerUser(
        workOrderAssignment.getEngineerUser() == null
            ? null
            : workOrderAssignment.getEngineerUser().getId());
  ...
  private WorkOrderAssignment mapToEntity(
    ...
    final WorkOrder workOrder =
        workOrderAssignmentDTO.getWorkOrder() == null
            ? null
            : workOrderRepository
                .findById(workOrderAssignmentDTO.getWorkOrder())
                .orElseThrow(() -> new NotFoundException("workOrder not found"));
    workOrderAssignment.setWorkOrder(workOrder);
    final User engineerUser =
        workOrderAssignmentDTO.getEngineerUser() == null
            ? null
            : userRepository
                .findById(workOrderAssignmentDTO.getEngineerUser())
  ...
  public void on(final BeforeDeleteWorkOrder event) {
    ...
    final WorkOrderAssignment workOrderWorkOrderAssignment =
        workOrderAssignmentRepository.findFirstByWorkOrderId(event.getId());
  ...
  public void on(final BeforeDeleteUser event) {
    final ReferencedException referencedException = new ReferencedException();
    final WorkOrderAssignment engineerUserWorkOrderAssignment =
        workOrderAssignmentRepository.findFirstByEngineerUserId(event.getId());
