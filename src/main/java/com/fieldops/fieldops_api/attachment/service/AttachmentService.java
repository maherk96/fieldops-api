...
public class AttachmentService {
    ...
    private AttachmentDTO mapToDTO(final Attachment attachment, final AttachmentDTO attachmentDTO) {
        ...
        attachmentDTO.setWorkOrder(attachment.getWorkOrder() == null ? null : attachment.getWorkOrder().getId());
        attachmentDTO.setEvent(attachment.getEvent() == null ? null : attachment.getEvent().getId());
        attachmentDTO.setUploadedByUser(attachment.getUploadedByUser() == null ? null : attachment.getUploadedByUser().getId());
    ...
    private Attachment mapToEntity(final AttachmentDTO attachmentDTO, final Attachment attachment) {
        ...
        final WorkOrder workOrder = attachmentDTO.getWorkOrder() == null ? null : workOrderRepository.findById(attachmentDTO.getWorkOrder())
                .orElseThrow(() -> new NotFoundException("workOrder not found"));
        attachment.setWorkOrder(workOrder);
        final WorkOrderEvent event = attachmentDTO.getEvent() == null ? null : workOrderEventRepository.findById(attachmentDTO.getEvent())
                .orElseThrow(() -> new NotFoundException("event not found"));
        attachment.setEvent(event);
        final User uploadedByUser = attachmentDTO.getUploadedByUser() == null ? null : userRepository.findById(attachmentDTO.getUploadedByUser())
    ...
    public void on(final BeforeDeleteWorkOrder event) {
        ...
        final Attachment workOrderAttachment = attachmentRepository.findFirstByWorkOrderId(event.getId());
    ...
    public void on(final BeforeDeleteUser event) {
        final ReferencedException referencedException = new ReferencedException();
        final Attachment uploadedByUserAttachment = attachmentRepository.findFirstByUploadedByUserId(event.getId());
