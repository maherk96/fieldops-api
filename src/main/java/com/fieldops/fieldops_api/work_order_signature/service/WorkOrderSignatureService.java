...
public class WorkOrderSignatureService {
    ...
    public List<WorkOrderSignatureDTO> findAll() {
        final List<WorkOrderSignature> workOrderSignatures = workOrderSignatureRepository.findAll(Sort.by("id"));
    ...
    public WorkOrderSignatureDTO get(final UUID id) {
        return workOrderSignatureRepository.findById(id)
                .map(workOrderSignature -> mapToDTO(workOrderSignature, new WorkOrderSignatureDTO()))
    ...
    public void update(final UUID id, final WorkOrderSignatureDTO workOrderSignatureDTO) {
        final WorkOrderSignature workOrderSignature = workOrderSignatureRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        mapToEntity(workOrderSignatureDTO, workOrderSignature);
    ...
    private WorkOrderSignatureDTO mapToDTO(final WorkOrderSignature workOrderSignature,
        ...
        workOrderSignatureDTO.setWorkOrder(workOrderSignature.getWorkOrder() == null ? null : workOrderSignature.getWorkOrder().getId());
        return workOrderSignatureDTO;
    }

    private WorkOrderSignature mapToEntity(final WorkOrderSignatureDTO workOrderSignatureDTO,
        ...
        final WorkOrder workOrder = workOrderSignatureDTO.getWorkOrder() == null ? null : workOrderRepository.findById(workOrderSignatureDTO.getWorkOrder())
    ...
    public void on(final BeforeDeleteWorkOrder event) {
        ...
        final WorkOrderSignature workOrderWorkOrderSignature = workOrderSignatureRepository.findFirstByWorkOrderId(event.getId());
