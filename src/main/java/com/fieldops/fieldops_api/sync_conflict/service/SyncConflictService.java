...
public class SyncConflictService {
    ...
    public SyncConflictService(final SyncConflictRepository syncConflictRepository,
    ...
    public void update(final UUID id, final SyncConflictDTO syncConflictDTO) {
        final SyncConflict syncConflict = syncConflictRepository.findById(id)
                .orElseThrow(NotFoundException::new);
        mapToEntity(syncConflictDTO, syncConflict);
    ...
    private SyncConflictDTO mapToDTO(final SyncConflict syncConflict,
            final SyncConflictDTO syncConflictDTO) {
        ...
        syncConflictDTO.setResolvedByUser(syncConflict.getResolvedByUser() == null ? null : syncConflict.getResolvedByUser().getId());
        return syncConflictDTO;
    }

    private SyncConflict mapToEntity(final SyncConflictDTO syncConflictDTO,
        ...
        final User resolvedByUser = syncConflictDTO.getResolvedByUser() == null ? null : userRepository.findById(syncConflictDTO.getResolvedByUser())
    ...
    public void on(final BeforeDeleteUser event) {
        ...
        final SyncConflict resolvedByUserSyncConflict = syncConflictRepository.findFirstByResolvedByUserId(event.getId());
