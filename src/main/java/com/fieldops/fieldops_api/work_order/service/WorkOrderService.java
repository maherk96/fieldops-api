...
public class WorkOrderService {
  ...
  private WorkOrderDTO mapToDTO(final WorkOrder workOrder, final WorkOrderDTO workOrderDTO) {
    ...
    workOrderDTO.setLocation(
        workOrder.getLocation() == null ? null : workOrder.getLocation().getId());
    workOrderDTO.setAsset(workOrder.getAsset() == null ? null : workOrder.getAsset().getId());
    workOrderDTO.setLastModifiedBy(
        workOrder.getLastModifiedBy() == null ? null : workOrder.getLastModifiedBy().getId());
    return workOrderDTO;
  }
  ...
  private WorkOrder mapToEntity(final WorkOrderDTO workOrderDTO, final WorkOrder workOrder) {
    ...
    final Location location =
        workOrderDTO.getLocation() == null
            ? null
            : locationRepository
                .findById(workOrderDTO.getLocation())
                .orElseThrow(() -> new NotFoundException("location not found"));
    workOrder.setLocation(location);
    final Asset asset =
        workOrderDTO.getAsset() == null
            ? null
            : assetRepository
                .findById(workOrderDTO.getAsset())
                .orElseThrow(() -> new NotFoundException("asset not found"));
    workOrder.setAsset(asset);
    final User lastModifiedBy =
        workOrderDTO.getLastModifiedBy() == null
            ? null
            : userRepository
                .findById(workOrderDTO.getLastModifiedBy())
  ...
  public void on(final BeforeDeleteUser event) {
    final ReferencedException referencedException = new ReferencedException();
    final WorkOrder lastModifiedByWorkOrder =
        workOrderRepository.findFirstByLastModifiedById(event.getId());
