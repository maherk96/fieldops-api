...
public class WorkOrderEventService {
  ...
  private WorkOrderEventDTO mapToDTO(
    ...
    workOrderEventDTO.setWorkOrder(
        workOrderEvent.getWorkOrder() == null ? null : workOrderEvent.getWorkOrder().getId());
    workOrderEventDTO.setCreatedByUser(
        workOrderEvent.getCreatedByUser() == null
            ? null
            : workOrderEvent.getCreatedByUser().getId());
  ...
  private WorkOrderEvent mapToEntity(
    ...
    final WorkOrder workOrder =
        workOrderEventDTO.getWorkOrder() == null
            ? null
            : workOrderRepository
                .findById(workOrderEventDTO.getWorkOrder())
                .orElseThrow(() -> new NotFoundException("workOrder not found"));
    workOrderEvent.setWorkOrder(workOrder);
    final User createdByUser =
        workOrderEventDTO.getCreatedByUser() == null
            ? null
            : userRepository
                .findById(workOrderEventDTO.getCreatedByUser())
  ...
  public void on(final BeforeDeleteWorkOrder event) {
    ...
    final WorkOrderEvent workOrderWorkOrderEvent =
        workOrderEventRepository.findFirstByWorkOrderId(event.getId());
  ...
  public void on(final BeforeDeleteUser event) {
    final ReferencedException referencedException = new ReferencedException();
    final WorkOrderEvent createdByUserWorkOrderEvent =
        workOrderEventRepository.findFirstByCreatedByUserId(event.getId());
